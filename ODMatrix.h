...
vector<array<int, 2>> make_pair(vector<int> pre_data){
    vector<array<int, 2>> od_data;
    array<int, 2> data{};
...
ODMatrix::ODMatrix() {
    ...
    string fname = "C:/Users/Ishan Gautam/Desktop/Carpool/data/odmatrix_data.txt";
    vector <int> pre_data = readfile(fname);

    cout<<"size of predata is "<<pre_data.size()<<endl;

    vector<array<int, 2>> od_data = make_pair(pre_data);
...
//            vector<array<int *, 2>> row;
    ...
    for (int i=0; i<matrix.size(); ++i){
        for(int j=0; j<matrix[i].size(); ++j){
            cout<<"the matrix data is ["<<matrix[i][j].at(0)<<", "<<matrix[i][j].at(1)<<"]"<<endl;
        }
    }
